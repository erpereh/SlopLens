...
- Popup mínimo: estado del backend, Auto Analyze, umbral resumido, tema y CTA “Open Dashboard”. Feed, idioma, reduced motion y providers se editan en el dashboard de la extensión (Options a pestaña completa: Overview, Feed, Providers, Appearance, Diagnostics).
...
- Backend local Hono con esas rutas (ya no son stubs 501), incluido `GET /metrics` de solo lectura (estados y conteos agregados, sin secrets ni texto).
- Settings de proveedores en el dashboard; keys en SecretStore del SO (archivo local solo como fallback etiquetado).
...
- dashboard de extensión como centro de configuración.
